...
which sets the absolute scale and position of the footprint.
...
def load_model(path: str) -> dict:
    ...
    return dict(path=path, band=band, W=img.width, H=img.height,
                ...
                E0=E0, N0=N0, alt=alt, epsg=epsg, lat=lat, lon=lon,
...
def main():
    ...
    ap.add_argument("--ground-elev", type=float, required=True,
                    help="ground elevation under the frame, m ASL (from a DEM)")
    ...
    H_utm, H_local = flat_ground_homography(m, args.ground_elev)
    ...
    for u, v in [(0, 0), (W, 0), (W, Hh), (0, Hh), (W / 2, Hh / 2)]:
        gc = np.array(pixel_to_ground(m, u, v, args.ground_elev))
    ...
    uc, vc = ground_to_pixel(m, *pixel_to_ground(m, W / 2, Hh / 2, args.ground_elev), args.ground_elev)
    print(f"pixel round-trip at centre = {np.hypot(uc - W / 2, vc - Hh / 2):.4f} px")

    if args.out:
        out, (nx, ny), (we, hn) = georeference_to_geotiff(m, args.ground_elev, args.out, args.gsd)
